...
fits_noro_full_vary_max_lag <- list()

fits_noro_full_vary_max_lag$geom[[1]] <-
  fits_noro_full_vary_max_lag$pois[[1]] <-
  fits_noro_full_vary_max_lag$lin[[1]] <-
  fits_noro_full_vary_max_lag$unres[[1]] <- hhh4(noroBE, ctrl_full)
...
for(max_lag in 2:7){
  ...
  # fit models:
  fits_noro_full_vary_max_lag$geom[[max_lag]] <- profile_par_lag(noroBE, ctrl_full)
  fits_noro_full_vary_max_lag$pois[[max_lag]] <- profile_par_lag(noroBE, ctrl_full_pois)

  # use grid-based optimization for linear lag:
  fits_noro_full_vary_max_lag$lin[[max_lag]] <- fit_par_lag(noroBE, ctrl_full_lin,
                                                            range_par = vals_par_lag_lin,
                                                            use_update = FALSE)$best_mod

  if(max_lag > 2){
    fits_noro_full_vary_max_lag$unres[[max_lag]] <-
      profile_par_lag(noroBE, ctrl_full_unres,
                      start_par_lag = c(fits_noro_full_vary_max_lag$unres[[max_lag - 1]]$par_lag, -2))
  }else{
    fits_noro_full_vary_max_lag$unres[[max_lag]] <-
      profile_par_lag(noroBE, ctrl_full_unres)
  }
...
for(lag_structure in c("pois", "lin", "geom", "unres")){
  for(max_lag in 1:7){
    AICs_vary_max_lag[max_lag, lag_structure] <-
      AIC(fits_noro_full_vary_max_lag[[lag_structure]][[max_lag]])
  }
}

# store:
save(fits_noro_full_vary_max_lag, file = paste0("model_fits/noro_", n_seas*52 , "/fits_noro_full_vary_max_lag_", n_seas*52, ".rda"))
write.csv(AICs_vary_max_lag, file = paste0("AIC/AIC_noro_full_vary_max_lag_", n_seas*52, ".csv"))
...
# initialize lists to store models:
fits_noro_gravity_vary_max_lag <- list()
fits_noro_gravity_vary_max_lag$geom[[1]] <-
  fits_noro_gravity_vary_max_lag$pois[[1]] <-
  fits_noro_gravity_vary_max_lag$lin[[1]] <-
  fits_noro_gravity_vary_max_lag$unres[[1]] <- hhh4(noroBE, ctrl_gravity)
...
for(max_lag in 2:7){
  ...
  # fit models:
  fits_noro_gravity_vary_max_lag$geom[[max_lag]] <- profile_par_lag(noroBE, ctrl_gravity)
  fits_noro_gravity_vary_max_lag$pois[[max_lag]] <- profile_par_lag(noroBE, ctrl_gravity_pois)

  # use grid-based optimization for linear lag:
  fits_noro_gravity_vary_max_lag$lin[[max_lag]] <- fit_par_lag(noroBE, ctrl_gravity_lin,
                                                               range_par = vals_par_lag_lin,
                                                               use_update = FALSE)$best_mod

  if(max_lag > 2){
    fits_noro_gravity_vary_max_lag$unres[[max_lag]] <-
      profile_par_lag(noroBE, ctrl_gravity_unres,
                      start_par_lag = c(fits_noro_gravity_vary_max_lag$unres[[max_lag - 1]]$par_lag, -2))
  }else{
    fits_noro_gravity_vary_max_lag$unres[[max_lag]] <-
      profile_par_lag(noroBE, ctrl_gravity_unres)
  }
...
for(lag_structure in c("pois", "lin", "geom", "unres")){
  for(max_lag in 1:7){
    AICs_gravity_vary_max_lag[max_lag, lag_structure] <-
      AIC(fits_noro_gravity_vary_max_lag[[lag_structure]][[max_lag]])
  }
}

# store:
save(fits_noro_gravity_vary_max_lag, file = paste0("model_fits/noro_", n_seas*52 ,"/fits_noro_gravity_vary_max_lag_", n_seas*52, ".rda"))
write.csv(AICs_gravity_vary_max_lag, file = paste0("AIC/AIC_noro_gravity_vary_max_lag_", n_seas*52, ".csv"))