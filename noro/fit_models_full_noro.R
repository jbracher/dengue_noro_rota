...
setwd("noro")
# scp johannes@130.60.71.234://home/johannes/Documents/hhh4predict/Theory/Article_Theory/data_analysis_forecasting/noro/fit_models_full_noro.R fit_models_full_noro.R

# create folder structure if necessary:
list.dirs()
dir.create("model_fits")
for(lag_structure in c("ar1", "geom", "pois", "lin", "unres")){
  dir.create(paste0("model_fits/noro_full_", lag_structure))
}
...
ctrl_full_unres$max_lag <- max_lag_unres
...
for(ind in tps){
  ...
  # fit ar1 model:
  fit_noro_full_ar1_temp <- hhh4(noroBE_temp, ctrl_full)
  save(fit_noro_full_ar1_temp, file = paste0("model_fits/noro_full_ar1",
                                             "/fit_noro_full_ar1_", ind, ".rda"))

  # fit model with geometric lags:
  start_par_lag <- ifelse(ind == tps[1], 0.5, fit_noro_full_geom_temp$par_lag)
  fit_noro_full_geom_temp <- profile_par_lag(noroBE_temp, ctrl_full,
                                             start_par_lag = start_par_lag)
  save(fit_noro_full_geom_temp, file = paste0("model_fits/noro_full_geom",
                                              "/fit_noro_full_geom_", ind, ".rda"))

  # fit model with Poisson lags:
  start_par_lag <- ifelse(ind == tps[1], 0.5, fit_noro_full_pois_temp$par_lag)
  fit_noro_full_pois_temp <- profile_par_lag(noroBE_temp, ctrl_full_pois,
                                             start_par_lag = start_par_lag)
  save(fit_noro_full_pois_temp, file = paste0("model_fits/noro_full_pois",
                                              "/fit_noro_full_pois_", ind, ".rda"))
  ...
  # start_par_lag <- ifelse(ind == tps[1], 0.5, fit_noro_full_lin_temp$par_lag)
  # fit_noro_full_lin_temp <- profile_par_lag(noroBE_temp, ctrl_full_lin,
  #                                           start_par_lag = start_par_lag)
  fit_noro_full_lin_temp <- fit_par_lag(noroBE_temp, ctrl_full_lin,
                                        range_par = vals_par_lag_lin, use_update = FALSE)$best_mod
  save(fit_noro_full_lin_temp, file = paste0("model_fits/noro_full_lin",
                                             "/fit_noro_full_lin_", ind, ".rda"))

  # fit model with unconstrained weights:
  start_par_lag <- if(ind == tps[1]){
    rep(0.5, max_lag_unres - 1)
  }else{
    fit_noro_full_unres_temp$par_lag
  }
  fit_noro_full_unres_temp <- profile_par_lag(noroBE_temp, ctrl_full_unres,
                                              start_par_lag = start_par_lag)
  save(fit_noro_full_unres_temp, file = paste0("model_fits/noro_full_unres",
                                               "/fit_noro_full_unres_", ind, ".rda"))
