# Analyse the log scores for the rotavirus forecasts
...
multiv_logS_glarma <- read.csv("logS/multiv_logS_rota_glarma.csv")
...
# store:
# write.csv(logS_horizons_rota, file = "logS/summary_multiv_logS_rota.csv", row.names = FALSE)
...
write.csv(pvals_ar1_vs_geom,
          file = "logS/pvals_logS_ar1_vs_geom_rota_horizons.csv", row.names = TRUE)
...
write.csv(pvals_gravity_vs_full,
          file = "logS/pvals_logS_gravity_vs_full_rota_horizons.csv", row.names = TRUE)


# Plot as in manuscript:
plot(1:4, logS_horizons_rota$ar1, type = "b", col = cols_models_nr["ar1"], pch = 16, cex = 0.9, lwd=1.5,
     ylim = c(1.76, 1.88), xlab = "forecast horizon", ylab = "mean multivariate logS", axes = FALSE,
     main = "(a) rotavirus")
axis(1, at = 1:4); axis(2); box()
lines(1:4, logS_horizons_rota$geom, type = "b", col = cols_models_nr["geom"], pch = 16, cex = 0.9, lwd=1.5)

lines(1:4, logS_horizons_rota$gravity_ar1, type = "b", col = cols_models_nr["ar1"],
      pch = 17, cex = 0.9, lty = 2, lwd=1.5)
lines(1:4, logS_horizons_rota$gravity_geom, type = "b", col = cols_models_nr["geom"],
      pch = 17, cex = 0.9, lty = 2, lwd=1.5)

lines(1:4, logS_horizons_rota$naive, type = "b", col = cols_models_nr["naive"], cex = 0.7, pch = 15)

lines(1:4, logS_horizons_rota$glarma, type = "b", col = cols_models_nr["glarma"], cex = 0.7, pch = 15)
