...
results_detailed <- data.frame(data_set = rep("rotaBE", n_units*max_horizon*length(tps)),
                               unit = 1,
...
for(unit in 1:n_units){
  ...
  for(ind in tps){
    ...
    pred_temp <- forecasting_nStepAhead_glarma(y = y[, unit], X = X, t_condition = ind,
                                               max_horizon = max_horizon, phiLags = 1:2,
                                               niter = 1000)
...
# write out results:
write.csv(multiv_log_scores_n_step_ahead, file = "logS/multiv_logS_rota_glarma.csv")
write.csv(results_detailed, file = "forecasts/forecasts_rota_glarma.csv")
